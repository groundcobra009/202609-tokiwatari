...
## Issue #3 — 投稿欄を本文中心へ整理（未マージ）

- 今の記録/未来の自分への質問を分離。公開設定とサンプル切替・使い方は折りたたみ、フィルタはフィード上へ移動。
- 今の記録は送信時刻で保存。未来モードの過去日付を拒否。比較と今日の一歩の既存導線を維持。
- スライド担当者向けの最新UI説明をSLIDES_HANDOFF.mdへ追加。
- 通常投稿が未来側へ分類される表示不具合も修正。check exit 0（mock/guest、10群）、Workers操作・390px表示を確認。公開版42ba304f-0fd8-4ebb-bf72-11bfadecf910へ反映。

## 2026-09-13 — Issue #3：一次審査用のスライド・動画制作ハンドオフ（未マージ）

- **変更対象**: SLIDES_HANDOFF.md、README、docs/codex-handoff.md。
- **変更内容**: 最新ユーザー共有事項と公式PDFの区別、公式5観点と根拠の対応、6枚の本文・ノート案、可視付録、実応答、3分動画の制作案、実装状態・提出前確認を記載。
- **補足設計**: ノート・音声だけの情報欠落を避け、審査に必要な証拠を読める本文・付録にも置く。透明文字や欄外への依存を避ける。
- **範囲**: 制作担当者へ渡す文書。スライド/動画の作成・送信は本変更に含まない。アプリコード変更なし。
